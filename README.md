...
Now we will define the TURN function. First our robot starts to move at 60% speed. Then, if the ultrasonic sensor 2 (The sensor that looks at the inner walls.) sees that the distance is 220 cm, it increases the round variable by 1. After that, we decrease our robots speed by 45% (making it 25%) to make the turn easier. Then, our steering mech starts to move at 100% power. This makes our robot's steering mech turn to right at full power. Our robot continues to turn to right until ultrasonic sensor 2 sees that the distance is 70 cm.
When the ultrasonic sensor sees 70 cm, the steering mech stops. If ultrasonic sensor doesn't see 220cm as distance, it instead
